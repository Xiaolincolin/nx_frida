...
function hook_235F4(baseAddr) {
    ...
    Interceptor.attach(sub_235F4, {
        onEnter(args) {
            ...
            console.log('[+] a1->', hexdump(a1, {length: 32}));
...
function hook_2349c(baseAddr) {
    ...
    Interceptor.attach(sub_2349C, {
        ...
        onLeave(retval) {
            console.log('[+] sub_2349C a5 Data:');
            try {
                const base = this.a5;
                console.log('readPointer =>', hexdump(base, {length: 16}))
                console.log('[+] sub_2349C a5->', base.toString(16));
                const flag = base.readU32();
                let length, dataPtr;

                if ((flag & 1) !== 0) {
                    // heap buffer
                    length = base.add(8).readU32();
                    dataPtr = base.add(16).readPointer();
                } else {
                    // inline buffer
                    length = flag >> 1;
                    dataPtr = base.add(8).readPointer();
                }

                if (length > 0 && dataPtr.isNull() === false) {
                    const content = Memory.readByteArray(dataPtr, length);
                    console.log('[+] Encrypted result (a5):\n' +
                        hexdump(content, {offset: 0, length}));
                } else {
                    console.warn('[!] Empty or invalid length');
                }
            } catch (e) {
                console.error('[!] Failed to read a5 result:', e);
            }
        }
    });
}
...
function hook_tmp(baseAddr) {
    ...
    Interceptor.attach(sub_22C24, {
        onEnter(args) {
            ...
            console.log('sub_22FD4 x24:', x8.toString(16))
        },
        onLeave(retval) {
        }
    });
